...
public class MyAccessDecisionManager implements AccessDecisionManager {
    ...
    public void decide(Authentication authentication, Object object, Collection<ConfigAttribute> configAttributes) throws AccessDeniedException, InsufficientAuthenticationException {
        ...
        while (iterator.hasNext()) {
            if (authentication == null) {throw new AccessDeniedException("授权信息为空");}
            ConfigAttribute configAttribute = iterator.next();
            //当前请求需要的权限
            String needRole = configAttribute.getAttribute();
            log.info(">>>>>>>权限校验(needRole):{}",needRole);
            //当前用户所具有的权限
            ...
            if("WXM_ROLE_NEED".equals(needRole)){
                throw new AccessDeniedException("请先配置接口权限!");
            }
            if("WXM_ROLE_CHECK_EXC".equals(needRole)){
                throw new AccessDeniedException("校验资源权限发生异常!");
            }
        }
