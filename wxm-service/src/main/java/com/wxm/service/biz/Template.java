...
public abstract class Template<T> {
    public abstract List<?> queryList() throws DbSvcException;

    public final List<T> service(Class<?> clazzSource, Class<T> clazzTarget, String msg) throws DbSvcException, BizSvcException {
        try {
            ...
            if (e instanceof DbSvcException) {
                throw (DbSvcException) e;
            }
            log.error("【{}】{}：{}", BizSvcEnum.BIZ_SVC_1001.toString(), BizSvcEnum.BIZ_SVC_1001.getMessage(msg), e.getMessage(), e);
            throw new BizSvcException(BizSvcEnum.BIZ_SVC_1001.toString(), BizSvcEnum.BIZ_SVC_1001.getMessage(msg),e.getMessage());
        }
    }

    public final List<T> service(Class<?> clazzSource, Class<T> clazzTarget, boolean useConverter, Converter converter, String msg) throws DbSvcException, BizSvcException {
        try {
            List<?> list = this.queryList();
            // 单例模式
            BeanUtils instance = BeanUtils.getInstance();
